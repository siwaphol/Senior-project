<?php
	...
	{
		$providers = array("" => "No Preference");
		foreach ($sso_providers as $provider => &$instance)
		{
			$providers[$provider] = $instance->DisplayName();
		}

		$tags = array("" => "No Preference");
		$result = $sso_db->Query("SELECT", array(
			"*",
			"FROM" => "?",
			"WHERE" => "enabled = 1",
		...
		{
			if ($row->tag_name != SSO_SITE_ADMIN_TAG || $sso_site_admin)  $tags[$row->id] = $row->tag_name;
		}

		if (isset($_REQUEST["id"]))
		{
			if ($_REQUEST["provider_name"] != "" && !isset($providers[$_REQUEST["provider_name"]]))  BB_SetPageMessage("error", "Please select a valid provider name from the options.");
			if ($_REQUEST["tag_id"] != "" && !isset($tags[$_REQUEST["tag_id"]]))  BB_SetPageMessage("error", "Please select a valid tag from the options.");
			...
			if (BB_GetPageMessageType() != "error")
			{
				...
				$optdesc = array();
				if ($_REQUEST["tag_id"] != "" && isset($tags[$_REQUEST["tag_id"]]))
				{
					$sqlfrom[] = "? AS ut";
					$sqlwhere[] = "u.id = ut.user_id";
					$sqlwhere[] = "ut.tag_id = ?";
					$sqlvars[] = $sso_db_user_tags;
					$sqlvars[] = $_REQUEST["tag_id"];
				}
				if ($_REQUEST["id"] != "")
				{
					$sqlwhere[] = "u.id = ?";
				...
				if ($_REQUEST["provider_name"] != "" && isset($providers[$_REQUEST["provider_name"]]))
				{
					$sqlwhere[] = "u.provider_name = ?";
					$sqlvars[] = $_REQUEST["provider_name"];
					$optdesc[] = htmlspecialchars(BB_Translate("Provider Name:  %s", $providers[$_REQUEST["provider_name"]]));
				}
				if ($_REQUEST["provider_id"] != "")
				{
					$sqlwhere[] = "u.provider_id = ?";
					$sqlvars[] = $_REQUEST["provider_id"];
					$optdesc[] = htmlspecialchars(BB_Translate("Provider ID:  %s", $_REQUEST["provider_id"]));
				}
				if ($_REQUEST["version"] !== "" && (int)$_REQUEST["version"] >= 0)
				{
					$sqlwhere[] = "u.version = ?";
					$sqlvars[] = (int)$_REQUEST["version"];
					$optdesc[] = htmlspecialchars(BB_Translate("Version:  %s", (int)$_REQUEST["version"]));
				}
				if ($_REQUEST["lastipaddr"] != "")
				{
					$sqlwhere[] = "u.lastipaddr = ?";
					$ipaddr = IPAddr::NormalizeIP($_REQUEST["lastipaddr"]);
					$sqlvars[] = $ipaddr["ipv6"];
					$optdesc[] = htmlspecialchars(BB_Translate("IP Address:  %s", $ipaddr["ipv6"] . ($ipaddr["ipv4"] != "" ? " (" . $ipaddr["ipv4"] . ")" : "")));
				}
				if ($_REQUEST["tag_id"] != "" && isset($tags[$_REQUEST["tag_id"]]))
				{
					$optdesc[] = htmlspecialchars(BB_Translate("Tag:  ", $tags[$_REQUEST["tag_id"]]));
				}

				foreach ($sso_fields as $key => $encrypted)
				{
					$md5key = md5($key);
					if (!$encrypted && isset($_REQUEST["field_search_" . $md5key]) && $_REQUEST["field_search_" . $md5key] != "")
					{
						$sqlwhere[] = "u.info LIKE ?";
						$sqlvars[] = "%" . $_REQUEST["field_search_" . $md5key] . "%";
						$optdesc[] = htmlspecialchars(BB_Translate("Field '%s' contains:  %s", $key, $_REQUEST["field_search_" . $md5key]));
					}
				}
				...
				if (isset($_REQUEST["output_provider_name"]) && $_REQUEST["output_provider_name"] == "yes")  $cols[] = "Provider Name";
				if (isset($_REQUEST["output_provider_id"]) && $_REQUEST["output_provider_id"] == "yes")  $cols[] = "Provider ID";
				if (isset($_REQUEST["output_version"]) && $_REQUEST["output_version"] == "yes")  $cols[] = "Version";
				if (isset($_REQUEST["output_lastipaddr"]) && $_REQUEST["output_lastipaddr"] == "yes")  $cols[] = "Last IP Address";
				if (isset($_REQUEST["output_tags"]) && $_REQUEST["output_tags"] == "yes")
				{
					$tagscol = count($cols);
					$cols[] = "Tags";
				}
				...
				foreach ($sso_fields as $key => $encrypted)
				{
					if (isset($_REQUEST["output_field_" . md5($key)]) && $_REQUEST["output_field_" . md5($key)] == "yes")
					{
						$cols[] = $key;
						$outputfields = true;
					}
				...
				while ($row = $result->NextRow())
				{
					...
					if (isset($_REQUEST["output_provider_name"]) && $_REQUEST["output_provider_name"] == "yes")  $row2[] = htmlspecialchars(isset($providers[$row->provider_name]) ? $providers[$row->provider_name] : $row->provider_name);
					if (isset($_REQUEST["output_provider_id"]) && $_REQUEST["output_provider_id"] == "yes")  $row2[] = htmlspecialchars($row->provider_id);
					if (isset($_REQUEST["output_version"]) && $_REQUEST["output_version"] == "yes")  $row2[] = $row->version;
					if (isset($_REQUEST["output_lastipaddr"]) && $_REQUEST["output_lastipaddr"] == "yes")  $row2[] = htmlspecialchars($row->lastipaddr);
					if (isset($_REQUEST["output_tags"]) && $_REQUEST["output_tags"] == "yes")
					{
						$row2[] = "_" . $row->id;
						$ids[] = $row->id;
					...
					if ($outputfields)
					{
						...
						foreach ($sso_fields as $key => $encrypted)
						{
							if (isset($_REQUEST["output_field_" . md5($key)]) && $_REQUEST["output_field_" . md5($key)] == "yes")  $row2[] = (isset($userinfo[$key]) ? htmlspecialchars($userinfo[$key]) : "&nbsp;");
						}
					}

					$row2[] = "<a href=\"" . BB_GetRequestURLBase() . "?action=edituser&id=" . htmlspecialchars($row->id) . "&sec_t=" . BB_CreateSecurityToken("edituser") . "\">Edit</a>";

					$rows[] = $row2;
				}

				// A second query is necessary if tags are displayed.
				if (count($ids))
				{
					$idmap = array();
					$result = $sso_db->Query("SELECT", array(
					...
					while ($row = $result->NextRow())
					{
						...
						{
							$key = "_" . $row->user_id;
							if (isset($idmap[$key]))  $idmap[$key] .= ", " . htmlspecialchars($tags[$row->tag_id]);
							else  $idmap[$key] = htmlspecialchars($tags[$row->tag_id]);
						}
					}
					...
					{
						$key = $row[$tagscol];
						if (isset($idmap[$key]))  $row[$tagscol] = $idmap[$key];
						else  $row[$tagscol] = "";
		...
		$contentopts = array(
			...
			"fields" => array(
				...
				array(
					...
					"name" => "provider_name",
					"options" => $providers,
					"select" => BB_GetValue("provider_name", ""),
					"htmldesc" => SSO_GenerateSearchOutputCheckbox("provider_name", true)
				),
				array(
					"title" => "Provider ID",
					"type" => "text",
					"width" => "15em",
					"name" => "provider_id",
					"value" => BB_GetValue("provider_id", ""),
					"htmldesc" => SSO_GenerateSearchOutputCheckbox("provider_id", true)
				),
				"startrow",
				array(
					"title" => "Account Version",
					...
					"width" => "15em",
					"name" => "version",
					"value" => BB_GetValue("version", ""),
					"htmldesc" => SSO_GenerateSearchOutputCheckbox("version", false)
				),
				array(
					"title" => "Last IP Address",
					"type" => "text",
					"width" => "15em",
					"name" => "lastipaddr",
					"value" => BB_GetValue("lastipaddr", ""),
					"htmldesc" => SSO_GenerateSearchOutputCheckbox("lastipaddr", false)
				),
				array(
					"title" => "Tag",
					"type" => "select",
					...
					"options" => $tags,
					"select" => BB_GetValue("tag_id", array()),
					"htmldesc" => SSO_GenerateSearchOutputCheckbox("tags", false)
				),
			),
			"submit" => "Search",
			"focus" => true
		...
		foreach ($sso_fields as $key => $encrypted)
		{
			...
			if ($encrypted)
			{
				$contentopts["fields"][] = array(
					...
					"type" => "custom",
					"width" => "15em",
					"value" => "<i>" . htmlspecialchars(BB_Translate("Encrypted field")) . "</i>",
					"htmldesc" => SSO_GenerateSearchOutputCheckbox("field_" . md5($key), false)
				);
			}
			else
			{
				$contentopts["fields"][] = array(
					...
					"type" => "text",
					"width" => "15em",
					"name" => "field_search_" . md5($key),
					"value" => BB_GetValue("field_search_" . md5($key), ""),
					"htmldesc" => SSO_GenerateSearchOutputCheckbox("field_" . md5($key), false)
				);
			}
