...
function SSO_Heartbeat() {
	jQuery('#sso_heartbeat').load('index.php', { 'sso_ajax' : 1, 'sso_id' : '<?php echo htmlspecialchars(BB_JSSafe($_REQUEST["sso_id"])); ?>', 'sso_action' : 'sso_heartbeat' });
}
...
<?php
	...
	function SSO_FrontendField($name)
	{
		...
		return "sso_" . md5(SSO_BASE_RAND_SEED2 . ":" . $sso_session_id[0] . ":" . $sso_apikey . ":" . $name);
	}
	...
	if (!isset($_REQUEST["sso_id"]))  SSO_DisplayError("Session ID expected.  Most likely causes:  Clicking a URL that launched a new web browser, using a non-offical client, or a bad or incorrectly configured web proxy.  If you clicked a URL in an e-mail, it opened a new web browser, and you got this error, then try this solution:  Copy the URL and paste it into the address bar of the other web browser.  Sorry for the inconvenience, but this behavior helps keep your account secure from hackers.");
...
try
{
	if (window.self !== window.top)  window.location.href = '<?php echo BB_JSSafe(BB_GetRequestHost() . SSO_ROOT_URL . "/index.php?sso_action=sso_iframe_error" . (isset($_REQUEST["lang"]) ? "&lang=" . urlencode($_REQUEST["lang"]) : "")); ?>';
	if (window.self !== window.parent)  window.location.href = '<?php echo BB_JSSafe(BB_GetRequestHost() . SSO_ROOT_URL . "/index.php?sso_action=sso_iframe_error" . (isset($_REQUEST["lang"]) ? "&lang=" . urlencode($_REQUEST["lang"]) : "")); ?>';
}
catch (ex)
{
}
...
<?php
		...
		else if (isset($_REQUEST["sso_action"]) && $_REQUEST["sso_action"] == "sso_validate")
		{
			...
			$sso_target_url = SSO_ROOT_URL . "/index.php?sso_action=sso_validate" . (isset($_REQUEST["lang"]) ? "&lang=" . urlencode($_REQUEST["lang"]) : "");
		...
		else if (isset($_REQUEST["sso_provider"]) && isset($sso_providers[$_REQUEST["sso_provider"]]))
		{
			...
			$sso_selectors_url = SSO_ROOT_URL . "/index.php" . (isset($_REQUEST["lang"]) ? "?lang=" . urlencode($_REQUEST["lang"]) : "");
			$sso_target_url = SSO_ROOT_URL . "/index.php?sso_provider=" . urlencode($sso_provider) . (isset($_REQUEST["lang"]) ? "&lang=" . urlencode($_REQUEST["lang"]) : "");
