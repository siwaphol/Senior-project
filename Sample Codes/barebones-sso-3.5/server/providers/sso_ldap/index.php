				...
				$contentopts = array(
					...
					"fields" => array(
						...
						array(
							...
							"desc" => "The LDAP Distinguished Name (DN) pattern to use to check logins against and load user information.  Should be in the format 'OU=users,DC=somewhere,DC=com' or similar.  The string 'CN=username' is prefixed for logins and used as a search filter for custom mapping."
						),
						array(
							"title" => "Map Username",
							"type" => "select",
						...
						array(
							...
							"desc" => "Remove domain prefix from the above mapped username."
						),
						array(
						...
						array(
							...
							"desc" => "Turn on debugging mode to get an idea of what LDAP fields are available for your LDAP server.  When enabled and a login is successful, this will output the fields and data of the user and then exit."
						),
					),
					"submit" => "Save",
					"focus" => true
		...
		public function ProcessFrontend()
		{
			...
			if (SSO_FrontendFieldValue("submit") !== false)
			{
				...
				else
				{
					...
					{
						$cn = "CN=" . str_replace(array("+", "\\", ";", ","), array("\\+", "\\\\", "\\;", "\\,"), $username);
						$dn = $cn . "," . $sso_settings["sso_ldap"]["dn"];
						...
						else
						{
							...
							$result = @ldap_search($ldap, $sso_settings["sso_ldap"]["dn"], $cn);
							if ($result === false && ldap_errno($ldap))  $extra = ldap_error($ldap);
							else
							{
						...
						else
						{
							...
							SSO_ActivateUser($origusername, serialize($sso_settings["sso_ldap"]), $mapinfo);
