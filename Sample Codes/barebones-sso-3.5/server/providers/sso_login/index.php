<?php
	...
	class sso_login_ModuleBase
	{
		...
		public function AddFindUserOptions(&$contentopts, &$options, &$num)
		{
		}

		public function AddFindUserCols(&$cols)
		{
			return false;
		}

		public function AddFindUserRowInfo(&$result, &$userinfo)
		{
		}
	...
	class sso_login extends SSO_ProviderBase
	{
		public function Init()
		{
			...
			if (!isset($sso_settings["sso_login"]["password_mode"]))  $sso_settings["sso_login"]["password_mode"] = "blowfish";
			if (!isset($sso_settings["sso_login"]["password_minrounds"]))  $sso_settings["sso_login"]["password_minrounds"] = (Blowfish::IsMcryptAvailable() ? 128 : 4);
		...
		public function MenuOpts()
		{
			...
			if ($sso_site_admin)
			{
				if ($sso_settings["sso_login"]["enabled"])
				{
					...
					$result["items"]["Find User"] = SSO_CreateConfigURL("finduser");
		...
		public function Config()
		{
			...
			if ($sso_site_admin && $sso_settings["sso_login"]["enabled"] && $_REQUEST["action2"] == "config")
			{
				if (isset($_REQUEST["configsave"]))
				{
					...
					$_REQUEST["password_minrounds"] = (int)$_REQUEST["password_minrounds"];
					...
					else if ($_REQUEST["password_minrounds"] < 1)  BB_SetPageMessage("error", "The 'Minimum Password Rounds' field contains an invalid value.");
					else if ($_REQUEST["password_mintime"] < 1)  BB_SetPageMessage("error", "The 'Minimum Password Time' field contains an invalid value.");
					...
					if (BB_GetPageMessageType() != "error")
					{
						...
						$sso_settings["sso_login"]["password_minrounds"] = $_REQUEST["password_minrounds"];
				...
				$options = array("blowfish" => "Blowfish::Hash() - A Bcrypt-like hash");
				...
				$contentopts["fields"][] = array(
					...
					"type" => "text",
					"name" => "password_minrounds",
					"value" => BB_GetValue("password_minrounds", $sso_settings["sso_login"]["password_minrounds"]),
					"desc" => "The minimum number of rounds required to hash a password.  Powers of 2 work best (e.g. 2 ^ 7 = 128)." . (Blowfish::IsMcryptAvailable() ? "" : "  The 'mcrypt' PHP library is not available, so keep the minimum number of rounds very low.  Due to the low number of minimum rounds, stored passwords will not be as secure as they should be.")
				);
				$contentopts["fields"][] = array(
					...
					"desc" => "The minimum amount of time, in milliseconds, required to spend to initially hash a password.  Only affects some password hashing modes."
				);
				...
				{
					$contentopts["fields"][] = array(
						"title" => BB_Translate("%s Module", $info["name"]),
						"type" => "accordion"
					);
					$contentopts["fields"][] = array(
				...
				$contentopts = array(
					...
					"fields" => array(
						array(
							...
							"desc" => "The fields to use to make up a unique registration system entry.  This can't be changed after installing.  The default is highly recommended."
						),
					),
					"submit" => "Install",
					"focus" => true
				...
				if (isset($_REQUEST["reset_password"]))
				{
					...
					if (BB_GetPageMessageType() != "error" && $_REQUEST["reset_password"] > 0)
					{
						if ($_REQUEST["reset_password"] == 1)
						{
							...
							$passwordinfo = self::HashPasswordInfo($data);
			...
			else if ($sso_settings["sso_login"]["enabled"] && $_REQUEST["action2"] == "finduser")
			{
				if (isset($_REQUEST["id"]))
				{
					$sqlfrom = array("?");
					$sqlwhere = array();
					$sqlvars = array($sso_db_sso_login_users);
					$optdesc = array();
					if ($_REQUEST["id"] != "")
					{
						$sqlwhere[] = "id = ?";
						$sqlvars[] = $_REQUEST["id"];
						$optdesc[] = htmlspecialchars(BB_Translate("User ID:  %s", $_REQUEST["id"]));
					}
					if (($sso_settings["sso_login"]["install_type"] == "email_username" || $sso_settings["sso_login"]["install_type"] == "username") && $_REQUEST["username"] != "")
					{
						$sqlwhere[] = "username LIKE ?";
						$sqlvars[] = "%" . $_REQUEST["username"] . "%";
						$optdesc[] = htmlspecialchars(BB_Translate("Username:  %s", $_REQUEST["username"]));
					}
					if ($sso_settings["sso_login"]["install_type"] == "email_username" || $sso_settings["sso_login"]["install_type"] == "email")
					{
						if ($_REQUEST["email"] != "")
						{
							$sqlwhere[] = "email LIKE ?";
							$sqlvars[] = "%" . $_REQUEST["email"] . "%";
							$optdesc[] = htmlspecialchars(BB_Translate("E-mail Address:  %s", $_REQUEST["email"]));
						}
						if ($_REQUEST["verified"] !== "")
						{
							$sqlwhere[] = "verified = ?";
							$sqlvars[] = $_REQUEST["verified"];
							$optdesc[] = htmlspecialchars(BB_Translate("Verified:  %u", (int)$_REQUEST["verified"]));
						}
					}
					if (!count($optdesc))  $optdesc[] = BB_Translate("All Entries");

					$desc = "<ul><li>" . implode("</li><li>", $optdesc) . "</li></ul>";

					$cols = array();
					if (isset($_REQUEST["output_id"]) && $_REQUEST["output_id"] == "yes")  $cols[] = "User ID";
					if ($sso_settings["sso_login"]["install_type"] == "email_username" || $sso_settings["sso_login"]["install_type"] == "username")
					{
						if (isset($_REQUEST["output_username"]) && $_REQUEST["output_username"] == "yes")  $cols[] = "Username";
					}
					if ($sso_settings["sso_login"]["install_type"] == "email_username" || $sso_settings["sso_login"]["install_type"] == "email")
					{
						if (isset($_REQUEST["output_email"]) && $_REQUEST["output_email"] == "yes")  $cols[] = "E-mail Address";
						if (isset($_REQUEST["output_verified"]) && $_REQUEST["output_verified"] == "yes")  $cols[] = "Verified";
					}

					$output = array();
					if (isset($_REQUEST["extra_output"]) && is_array($_REQUEST["extra_output"]))
					{
						foreach ($_REQUEST["extra_output"] as $val)  $output[$val] = $val;
					}
					$_REQUEST["extra_output"] = $output;

					$outputfields = false;
					if (isset($_REQUEST["extra_output"]["numrounds"]))
					{
						$cols[] = "Rounds";
						$outputfields = true;
					}
					...
					foreach ($g_sso_login_modules as $key => $info)
					{
						if ($sso_settings["sso_login"]["modules"][$key]["_a"])
						{
							$module = "sso_login_module_" . $key;
							$instance = new $module;
							if ($instance->AddFindUserCols($cols))
							{
								$outputfields = true;
								$modules[] = $instance;
							}
					...
					while ($row = $result->NextRow())
					{
						...
						if (isset($_REQUEST["output_id"]) && $_REQUEST["output_id"] == "yes")  $row2[] = $row->id;
						if ($sso_settings["sso_login"]["install_type"] == "email_username" || $sso_settings["sso_login"]["install_type"] == "username")
						{
							if (isset($_REQUEST["output_username"]) && $_REQUEST["output_username"] == "yes")  $row2[] = htmlspecialchars($row->username);
						}
						if ($sso_settings["sso_login"]["install_type"] == "email_username" || $sso_settings["sso_login"]["install_type"] == "email")
						{
							if (isset($_REQUEST["output_email"]) && $_REQUEST["output_email"] == "yes")  $row2[] = htmlspecialchars($row->email);
							if (isset($_REQUEST["output_verified"]) && $_REQUEST["output_verified"] == "yes")  $row2[] = htmlspecialchars(BB_Translate($row->verified ? "Yes" : "No"));
						}
						...
						if ($outputfields)
						{
							...
							if (isset($_REQUEST["extra_output"]["numrounds"]))  $row2[] = number_format($userinfo["rounds"], 0);

							foreach ($modules as $instance)  $instance->AddFindUserRowInfo($row2, $userinfo);
						}

						$row2[] = SSO_CreateConfigLink("Edit", "edituser", array("id" => $row->id));
					...
					BB_GeneratePage(BB_Translate("%s Search Results", $this->DisplayName()), $sso_menuopts, $contentopts);

					exit();
				}

				$contentopts = array(
					"desc" => BB_Translate("Find a user in the %s provider.", $this->DisplayName()),
					"nonce" => "action",
					"hidden" => array(
						"action" => "config",
						"provider" => "sso_login",
					...
					"fields" => array(
						...
						array(
							...
							"width" => "15em",
							"name" => "id",
							"value" => BB_GetValue("id", ""),
							"htmldesc" => SSO_GenerateSearchOutputCheckbox("id", true)
						),
					),
					"submit" => "Search",
					"focus" => true
				...
				if ($sso_settings["sso_login"]["install_type"] == "email_username" || $sso_settings["sso_login"]["install_type"] == "username")
				{
					$contentopts["fields"][] = array(
						...
						"htmldesc" => SSO_GenerateSearchOutputCheckbox("username", true)
					);
				...
				if ($sso_settings["sso_login"]["install_type"] == "email_username" || $sso_settings["sso_login"]["install_type"] == "email")
				{
					$contentopts["fields"][] = array(
						...
						"htmldesc" => SSO_GenerateSearchOutputCheckbox("email", true)
					);
					$num++;
					if ($num % 3 == 0)  $contentopts["fields"][] = "startrow";
					$contentopts["fields"][] = array(
						"title" => "Verified",
						"type" => "select",
						"width" => "15em",
						"name" => "verified",
						"options" => array("" => "No Preference", "1" => "Yes", "0" => "No"),
						"select" => BB_GetValue("verified", ""),
						"htmldesc" => SSO_GenerateSearchOutputCheckbox("verified", true)
					);
					$num++;
				}

				$options = array();
				$options["numrounds"] = "Number of password hashing rounds";
				foreach ($g_sso_login_modules as $key => $info)
				{
					if ($sso_settings["sso_login"]["modules"][$key]["_a"])
					{
						$module = "sso_login_module_" . $key;
						$instance = new $module;
						$instance->AddFindUserOptions($contentopts, $options, $num);
					}
				}
				...
				$contentopts["fields"][] = array(
					...
					"name" => "extra_output",
					"options" => $options,
					"desc" => "Select any additional output options."
				);

				BB_GeneratePage(BB_Translate("Find %s User", $this->DisplayName()), $sso_menuopts, $contentopts);
			}
...
<?php
		...
		public static function HashPasswordInfo($data)
		{
			global $sso_settings;

			if ($sso_settings["sso_login"]["password_mode"] == "password_hash_bcrypt" && function_exists("password_hash"))
			{
				// Calculate bits.
				$bits = 0;
				$x = 1;
				while ($x < $sso_settings["sso_login"]["password_minrounds"])
				{
					$bits++;
					$x *= 2;
				}
			...
			{
				$result = Blowfish::Hash($data, $sso_settings["sso_login"]["password_minrounds"], $sso_settings["sso_login"]["password_mintime"]);
			}
...
<?php
			...
			else if (isset($_REQUEST["sso_login_action"]) && $_REQUEST["sso_login_action"] == "signup_check" && $sso_settings["sso_login"]["open_reg"])
			{
				...
				if (!count($result["errors"]) && !count($result["warnings"]))
				{
					...
					else if ($result["htmlsuccess"] != "")  echo "<div class=\"sso_main_formokay\">" . $result["htmlsuccess"] . "</div>";
				}
			}
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							if (!count($messages["errors"]))
							{
								SSO_ActivateUser($userrow->id, $userinfo["extra"], $mapinfo);
					...
					else
					{
						...
						if (!count($messages["errors"]))
						{
							...
							if (!count($messages["errors"]))
							{
								...
								else if ($sso_settings["sso_login"]["require_two_factor"] || (isset($userinfo["two_factor_method"]) && $userinfo["two_factor_method"] != ""))
								{
