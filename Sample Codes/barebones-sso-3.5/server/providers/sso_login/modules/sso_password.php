<?php
	...
	class sso_login_module_sso_password extends sso_login_ModuleBase
	{
		...
		{
			$info = $this->GetInfo();
			if ($info["expire"] > 0)  $options["sso_password_expires"] = "Password expiration timestamp";
		}

		public function AddFindUserCols(&$cols)
		{
			$this->info = $this->GetInfo();
			if ($this->info["expire"] <= 0)  return false;

			$cols[] = "Password Expires";
		...
		{
			$result[] = (isset($userinfo["sso_password"]) ? htmlspecialchars(BB_FormatTimestamp("M j, Y @ g:i A", CSDB::ConvertFromDBTime($userinfo["sso_password"]["expires"]))) : BB_Translate("<i>Missing</i>"));
		}
