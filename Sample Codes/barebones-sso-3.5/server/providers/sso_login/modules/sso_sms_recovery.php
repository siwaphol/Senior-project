<?php
	...
	class sso_login_module_sso_sms_recovery extends sso_login_ModuleBase
	{
		...
		public function AddFindUserOptions(&$contentopts, &$options, &$num)
		{
			...
			if (is_object($data))
			{
				$options["sso_sms_recovery_phone"] = "SMS recovery phone number";
				$options["sso_sms_recovery_carrier"] = "SMS recovery carrier";
			}
		}

		public function AddFindUserCols(&$cols)
		{
			if (!isset($_REQUEST["extra_output"]["sso_sms_recovery_phone"]) && !isset($_REQUEST["extra_output"]["sso_sms_recovery_carrier"]))  return false;

			$data = @json_decode(@file_get_contents(SSO_ROOT_PATH . "/" . SSO_SUPPORT_PATH . "/sms_mms_gateways.txt"));
			if (!is_object($data))  return false;

			$this->sms_gateway_data = $data;
			if (isset($_REQUEST["extra_output"]["sso_sms_recovery_phone"]))  $cols[] = "SMS Phone";
			if (isset($_REQUEST["extra_output"]["sso_sms_recovery_carrier"]))  $cols[] = "SMS Carrier";
		...
		{
			if (isset($_REQUEST["extra_output"]["sso_sms_recovery_phone"]))
			{
				$result[] = (isset($userinfo["sso_sms_recovery"]) ? htmlspecialchars(preg_replace("/[^0-9]/", "", $userinfo["sso_sms_recovery"]["phone"])) : BB_Translate("<i>Missing</i>"));
			}
			if (isset($_REQUEST["extra_output"]["sso_sms_recovery_carrier"]))
			{
				if (!isset($userinfo["sso_sms_recovery"]))  $result[] = BB_Translate("<i>Missing</i>");
				else
				{
					$info = explode("-", $userinfo["sso_sms_recovery"]["carrier"]);
					if (count($info) != 2)  $result[] = BB_Translate("<i>Mangled</i>");
					else
					{
						$country = $info[0];
						$carrier = $info[1];
						if (!isset($this->sms_gateway_data->sms_carriers->$country) || !isset($this->sms_gateway_data->sms_carriers->$country->$carrier))  $result[] = BB_Translate("<i>Invalid</i>");
						else
						{
							$item = $this->sms_gateway_data->sms_carriers->$country->$carrier;

							$result[] = $this->sms_gateway_data->countries->$country . " - " . $item[0];
						}
					}
				}
			}
