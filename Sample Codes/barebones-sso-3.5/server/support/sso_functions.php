<?php
	...
	function SSO_AppendIPFields(&$contentopts, $info, $full = false)
	{
		...
		$contentopts["fields"][] = array(
			...
			"htmldesc" => "Enter one or more DNSRBL entries (domain[|required_response|alternate_response|...]).  One entry per line."
		);
		$contentopts["fields"][] = array(
	...
	function SSO_GenerateSearchOutputCheckbox($name, $checked)
	{
		return "<input type=\"checkbox\" id=\"output_" . $name . "\" name=\"output_" . $name . "\" value=\"yes\"" . ($checked ? " checked" : "") . " /> <label for=\"output_" . $name . "\">" . BB_Translate("Include in output") . "</label>";
	}
	...
	function SSO_ActivateNamespaceUser()
	{
		...
		if (!isset($sso_settings[""]["namespacekey2"]))  return false;
		if (!isset($sso_namespaces) && isset($_COOKIE["sso_server_ns"]))
		{
			$sso_namespaces = @base64_decode($_COOKIE["sso_server_ns"]);
			if ($sso_namespaces !== false)  $sso_namespaces = Blowfish::ExtractDataPacket($sso_namespaces, pack("H*", $sso_settings[""]["namespacekey"]), array("mode" => "CBC", "iv" => pack("H*", $sso_settings[""]["namespaceiv"]), "key2" => pack("H*", $sso_settings[""]["namespacekey2"]), "iv2" => pack("H*", $sso_settings[""]["namespaceiv2"]), "lightweight" => true));
			if ($sso_namespaces !== false)  $sso_namespaces = @unserialize($sso_namespaces);
		}
		if (!isset($sso_namespaces) || $sso_namespaces === false)  $sso_namespaces = array();
	...
	function SSO_ActivateUser($id, $entropy, $info, $automate = false, $activatesession = true)
	{
		global $sso_rng, $sso_db, $sso_db_users, $sso_db_user_tags, $sso_db_tags, $sso_provider, $sso_ipaddr;
...
<?php
	...
	{
		global $sso_rng, $sso_db, $sso_db_user_sessions, $sso_db_temp_sessions, $sso_session_info, $sso_namespaces, $sso_apirow, $sso_sessionrow, $sso_sessionrow2, $sso_ipaddr, $sso_settings;
		...
		try
		{
			...
			{
				$sso_namespaces[$sso_apirow->namespace] = $_COOKIE["sso_server_id2"];
				$data = serialize($sso_namespaces);
