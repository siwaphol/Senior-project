...
$head = iconv('UTF-8', 'TIS-620', "ข้อมูลนักศึกษาที่ลงทะเบียน ภาคเรียนที่ $semester ปีการศึกษา $year"); 
$id = iconv('UTF-8', 'TIS-620', "ที่");
$code = iconv('UTF-8', 'TIS-620', "รหัสนักศึกษา");
$fullname = iconv('UTF-8', 'TIS-620', "ชื่อ - นามสกุล");
$pw = iconv('UTF-8', 'TIS-620', "รหัสผ่าน");


	$sql1 = "SELECT s.studentId As studentId,studentName,studentPw FROM student As s,register As r WHERE s.studentId=r.studentId AND r.courseId='$course' AND r.sectionId='$section' ORDER BY s.studentId ASC";
	...
	$row3 = mysql_fetch_array($result3);
			...
			// password
			$pass =& $workbook->addformat();
            $pass->set_font('Angsana New');
            $pass->set_size(14);
            $pass->set_color('black');
            $pass->set_align('center');
            $pass->set_text_v_align(1);
			$pass->set_num_format('0000');
			// date
            ...
            $worksheet->set_column('A:B', 2.00);
            $worksheet->set_column('B:C', 4.00);
            $worksheet->set_column('C:D', 11.29);
            $worksheet->set_column('D:E', 21.00);
            $worksheet->set_column('E:F', 5.00);
            $worksheet->set_column('F:G', 20.00);
            ...
            $worksheet->write_blank(E7,$table_header);
            $worksheet->write(F7,"$pw", $table_header);
            $xlsRow = 8;

            # ตรงนี้คือดึงข้อมูลจาก mysql มาใส่ใน Cell
                while(list($studentId,$studentName,$studentPw)=mysql_fetch_row($result1)) {
                    ++$i;
                        $worksheet->write("B$xlsRow", "$i", $xlsCellDesc_center);
                        $worksheet->write("C$xlsRow", "$studentId", $xlsCellDesc_center); 
                        $worksheet->write("D$xlsRow", "$studentName", $xlsCellDesc_left);
                        $worksheet->write_blank("E$xlsRow", $xlsCellDesc_left);
                        $worksheet->write("F$xlsRow", "$studentPw", $pass); 
                $xlsRow++;
                }
...
header("Content-Type: application/x-msexcel; name=\"$course-$section.xls\"");
header("Content-Disposition: attachment; filename=\"$course-$section.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
