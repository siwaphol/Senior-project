...
mysql_db_query("project499","SET CHARACTER SET tis620");
$course = $_GET['course'];
$section = $_GET['sec'];
$year = $_GET['year'];
$semester = $_GET['sem'];
...
$head = iconv('UTF-8', 'TIS-620', "รายงานผลสรุปคะแนนส่งงานของนักศึกษา ภาคเรียนที่ $semester ปีการศึกษา $year"); 
            ...
            // ตรงนี้คือดึงข้อมูลจาก mysql มาใส่ใน Cell
                while($row1 = mysql_fetch_array($result1)) {
					...
					while($hwid = mysql_fetch_array($result4)){
						...
						$sql5 = "SELECT score FROM homework_sending 
						...
						$row5 = mysql_fetch_array($result5);
							if(mysql_num_rows($result5) == 0 || $row5['score'] == 0)
								$score = 0;
							else
								$score = $row5['score'];
...
header("Content-Type: application/x-msexcel; name=\"summary_$course-$section.xls\"");
header("Content-Disposition: attachment; filename=\"summary_$course-$section.xls\"");
$fh=fopen($fname, "rb");
fpassthru($fh);
unlink($fname);
